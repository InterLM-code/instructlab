...
DEFAULT_PROMPT_TEMPLATE_QWEN = """
<|im_start|>你是一个非常博学的人工智能助手，将忠实地帮助用户完成他们的任务。你需要在{{taxonomy}}{{" for the task \\"%s\\""|format(task_description)  if task_description}}下提出一组5个不同的任务指令。这些任务指令将被赋予一个GPT模型，我们将评估GPT模型以完成这些指令。
需求如下。
1. 尽量不要在每条指令中重复使用动词，以最大限度地增加多样性。
2. 作为指令的语言也应该多样化。例如，你应该将问题与命令式指令结合起来。
{% if not document -%}
3. 指令类型不应该具有主题各异的。列表应该遵循相同的主题和类别。
{% else -%}
3. 指令的类型应该与提供的示例类似。生成的指令和输出应该以提供的文档为基础。
{% endif -%}
4. 一个GPT语言模型应该能够完成指令。例如，不要要求助手创建任何视觉或音频输出。再比如，不要让助手在下午5点叫醒你或设置提醒，因为它无法执行任何操作。
5. 说明书应该是中文的。
6. 说明应该是1到2句话。祈使句或疑问句都可以。
{% if not document -%}
7. 你应该为指令生成适当的输入。输入字段应该包含为该指令提供的特定示例。它应该包含真实的数据，不应该包含简单的占位符。输入应提供大量内容，使指令具有挑战性，但理想情况下不应超过100个单词。
8. 并非所有指令都需要输入。例如，当一个指令询问一些一般信息时，“世界上最高的山峰是什么”，就没有必要提供具体的上下文。在这个例子中，我们简单地将"<noinput>"放在输入字段中。
9. 输出应该是对指令和输入的适当响应。确保输出不超过100个单词。
{% else -%}
7. 输出应该是对输入和指令的适当响应。长输出更可取。
{% endif %}
{% if not document -%}
5个任务的列表：
{% else -%}
